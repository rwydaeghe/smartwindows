...
class PeriodicBoundary(Boundary):
    ...
    def _register_grid(
        ...
        super()._register_grid(grid=grid, x=x, y=y)
...
class PML(Boundary):
    ...
    Note:
        ...
        subclasses: _PMLXlow, _PMLYlow or _PMLXhigh, _PMLYhigh,
        depending on the position in the grid.
    """
    ...
    def _calculate_parameters(self, thickness: int = 10):
        ...
        # set orientation dependent parameters: (different for x, y-PML)
        ...
        self.phi_E = np.zeros((Nx, Ny, 3))
        self.phi_H = np.zeros((Nx, Ny, 3))
        self.psi_Ex = np.zeros((Nx, Ny, 3))
        self.psi_Ey = np.zeros((Nx, Ny, 3))
        self.psi_Hx = np.zeros((Nx, Ny, 3))
        self.psi_Hy = np.zeros((Nx, Ny, 3))

        self.bE = np.exp(-(self.sigmaE / self.k + self.a) * self.grid.courant_number)
        self.cE = (
            (self.bE - 1.0)
        ...
        self.bH = np.exp(-(self.sigmaH / self.k + self.a) * self.grid.courant_number)
        self.cH = (
            (self.bH - 1.0)
...
class _PMLXlow(PML):
    ...
    def _set_sigmaE(self):
        sigma = self._sigma(np.arange(self.thickness - 0.5, -0.5, -1.0))
        self.sigmaE = np.zeros((self.thickness, self.grid.Ny, self.grid.Nz, 3))
        self.sigmaE[:, :, :, 0] = sigma[:, None, None]

    def _set_sigmaH(self):
        sigma = self._sigma(np.arange(self.thickness - 1.0, 0, -1.0))
...
class _PMLXhigh(PML):
    ...
    def _set_sigmaE(self):
        sigma = self._sigma(np.arange(0.5, self.thickness + 0.5, 1.0))
        self.sigmaE = np.zeros((self.thickness, self.grid.Ny, self.grid.Nz, 3))
        self.sigmaE[:, :, :, 0] = sigma[:, None, None]

    def _set_sigmaH(self):
        sigma = self._sigma(np.arange(1.0, self.thickness, 1.0))
        self.sigmaH = np.zeros((self.thickness, self.grid.Ny, self.grid.Nz, 3))
        self.sigmaH[:-1, :, :, 0] = sigma[:, None, None]
...
class _PMLZlow(PML):
    ...
    def _set_sigmaE(self):
        sigma = self._sigma(np.arange(self.thickness - 0.5, -0.5, -1.0))
        self.sigmaE = np.zeros((self.grid.Nx, self.grid.Ny, self.thickness, 3))
        self.sigmaE[:, :, :, 2] = sigma[None, None, :]

    def _set_sigmaH(self):
        sigma = self._sigma(np.arange(self.thickness - 1.0, 0, -1.0))
        self.sigmaH = np.zeros((self.grid.Nx, self.grid.Ny, self.thickness, 3))
